...
import dashboard.service.repositories.*;
...
public class ApplicationBootstrap {
	...
	public void init() {
		...
		story91.addTask("jira-hybris:CANADIENS-400");
		story91.addTask("Re factor 2 OLD methods PS: Separate Geo-Location", 4 * 60 * 60);
		story91.addTask("Mockito", 4 * 60 * 60);
		story91.addTask("Getting to know the code", 4 * 60 * 60);
		story91.addTask("Create test data", 4 * 60 * 60);
		...
		story88.addTask("UI Paging and sorting", 4 * 60 * 60);
		...
		story65.addTask("Service to get list of location in the DB", 4 * 60 * 60);
		this.userStoryAPI.save(story65);
		sprint.addUserStory(story65);

		sprint.addParticipant(participantAPI.create("philippe.lague-morin@hybris.com", "Philippe Lague-Morin", 6), Role.SCRUMMASTER);
